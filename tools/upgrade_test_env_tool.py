...
def _build_ssh_command(host: str, ssh_user: str, password_file: str, remote_command: str) -> list[str]:
    ...
    if not sshpass:
        raise RuntimeError(
            "sshpass is not installed on the Hermes host. "
            "Install sshpass or provide an alternative protected executor."
        )
